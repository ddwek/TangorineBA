...
Board board (true, "./zero_board_two_regular_cells_with_vertical_equal_shapes");

int on_tick_cb (GtkWidget *widget, GdkFrameClock *frame_clock, gpointer data)
{
	static int i = 3, j = 0, cnt = 0;
	static shape_t shape_0 = SHAPE_SUN, shape_1 = SHAPE_SUN;
	bool finished = false;
	int ret = -1;
	...
	if (!(gdk_frame_clock_get_frame_counter (frame_clock) & 0xf)) {
		if (shape_0 == SHAPE_SUN && shape_1 == SHAPE_SUN) {
			shape_0 = SHAPE_MOON;
			shape_1 = SHAPE_MOON;
		} else {
			shape_0 = SHAPE_SUN;
